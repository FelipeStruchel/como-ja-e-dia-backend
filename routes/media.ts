...
function buildUrls(type: string, filename: string, scope: string) {
  ...
  const prefix = scope === "daily" ? "/daily_vid" : "/media";
  const typeSegmentMap: Record<string, string> = { image: "images", video: "videos", text: "texts" };
  ...
  return { url: `${baseInternal}${rel}`, urlPublic: `${basePublic}${rel}` };
}
...
function createUploadMiddleware() {
  ...
  const fileFilter = (
    ...
    if (file.mimetype.startsWith("image/") && allowedImageTypes.includes(file.mimetype)) {
      ...
      cb(new Error("Tipo de arquivo não permitido. Use apenas imagens (JPG, PNG, GIF) ou vídeos (MP4, MOV, AVI, MKV)."));
    }
  };
...
export function registerMediaRoutes(
  app: Express,
  _deps: { MEDIA_TYPES: typeof MEDIA_TYPES; saveMedia: typeof saveMedia; listAllMedia: typeof listAllMedia }
) {
  ...
  app.get("/media", requireAuth, requireRole("bom_dia_admin"), async (req, res) => {
    try {
      const type = req.query.type as string | undefined;
      const scope = (req.query.scope || "media").toString();
      const baseFolder = resolveBaseFolder(scope);
      if (type && !Object.values(MEDIA_TYPES).includes(type as never)) {
        return res.status(400).json({ error: "Tipo de mídia inválido" });
      }
      ...
      const mediaWithUrls = media.map((item) => {
        ...
        return { ...item, url: urls.url, urlPublic: urls.urlPublic };
      });
  ...
  app.delete("/media/:type/:filename", requireAuth, requireRole("bom_dia_admin"), async (req, res) => {
    try {
      const { type, filename } = req.params;
      const scope = (req.query.scope || "media").toString();
      const baseFolder = resolveBaseFolder(scope);
